...
export default function Sidebar() {
  ...
  return (
    <aside className="flex flex-col bg-[#141820] border-r border-[#1e2535] select-none" style={{ width: 'var(--sidebar-width)', minWidth: 'var(--sidebar-width)' }}>
      {/* Logo — кликабельный, ведёт на главную страницу */}
      <button
        onClick={() => navigate('/')}
        onMouseEnter={() => setLogoHovered(true)}
        onMouseLeave={() => setLogoHovered(false)}
        className="flex items-center gap-2 px-4 py-5 border-b border-[#1e2535] w-full text-left transition-colors hover:bg-[#1a2035] group"
        title="На главную"
      >
        <div
      ...
      <button
        ...
        className="px-4 py-3 border-t border-[#1e2535] text-left hover:bg-[#1e2535] transition-colors group"
      >

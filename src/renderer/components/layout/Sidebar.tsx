...
export default function Sidebar({ collapsed: controlledCollapsed, onCollapsedChange, webResponsive = false }: { collapsed?: boolean; onCollapsedChange?: (collapsed: boolean) => void; webResponsive?: boolean }) {
  ...
  return (
    <aside
      ...
      className={`bx-app-sidebar relative z-20 flex-shrink-0 select-none flex-col border-r border-bx-border bg-bx-surface ${webResponsive ? 'hidden md:flex' : 'flex'}`}
      style={{ width: collapsed ? 72 : 240, minWidth: collapsed ? 72 : 240 }}
      aria-label="Основная навигация"
    >
      <div
        data-testid="sidebar-header"
        className={`bx-app-sidebar__header flex flex-shrink-0 items-center border-b border-bx-border ${collapsed ? 'h-28 flex-col justify-center gap-1 px-2 py-2' : 'h-16 gap-2 px-3'}`}
      >
        <button
          onClick={() => navigate('/dashboard')}
          className={`bx-app-sidebar__brand group flex min-h-11 min-w-0 items-center rounded-xl outline-none transition-colors ${collapsed ? 'h-11 w-11 justify-center' : 'flex-1 gap-2.5 px-1.5 text-left'}`}
          title={collapsed ? 'BX · На главную' : undefined}
          aria-label="BX — перейти на рабочий стол"
        >
          <span data-testid="bx-brand-mark" className="bx-app-brand-mark grid h-9 w-9 flex-shrink-0 place-items-center rounded-xl text-[11px] font-black tracking-tight text-bx-on-accent">BX</span>
        ...
        <button
          ...
          className="bx-app-icon-button grid h-11 w-11 flex-shrink-0 place-items-center rounded-xl border border-bx-border bg-bx-bg text-bx-muted outline-none transition-colors"
      ...
      <div className="flex-shrink-0 border-t border-bx-border p-2">
        {!collapsed && <div className="bx-app-sidebar__meta mb-2 rounded-xl border border-bx-border px-3 py-2.5"><p className="text-[8px] font-extrabold uppercase tracking-[0.14em] text-bx-accent">Рабочее пространство</p><p className="mt-1 text-[9px] leading-relaxed text-bx-muted">Навигация BX · версия {APP_VERSION}</p></div>}
        <button onClick={() => setAboutOpen(true)} className={`bx-app-sidebar__about flex min-h-11 w-full items-center rounded-xl text-bx-muted outline-none transition-colors ${collapsed ? 'justify-center' : 'gap-2.5 px-3 text-left'}`} title={collapsed ? `О программе · ${APP_VERSION}` : undefined} aria-label={`О программе. Версия ${APP_VERSION}`}>
...
function NavIcon({ name, active, collapsed }: { name: string; active: boolean; collapsed: boolean }) {
  return <span className={`bx-app-nav-icon grid flex-shrink-0 place-items-center rounded-lg transition-colors ${collapsed ? 'h-9 w-9' : 'h-7 w-7'} ${active ? 'bg-bx-on-accent/10 text-bx-on-accent' : 'bg-bx-bg text-bx-muted group-hover:text-bx-accent'}`}><Icon name={name} className="h-4 w-4" /></span>
}

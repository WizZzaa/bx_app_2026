    ...
    <div className="flex flex-1 p-6 gap-6 overflow-hidden bg-bx-bg text-bx-text font-sans">
      ...
      {hoveredDay && (
        <div
          ...
          className="z-50 w-72 bg-slate-900 border border-white/10 rounded-2xl p-4 shadow-xl backdrop-blur-md text-slate-100 pointer-events-none transition-all duration-100 ease-out flex flex-col gap-3"
        >
          <div className="flex items-center justify-between border-b border-white/5 pb-2 text-[10px] font-black text-slate-400">
            <span>
          ...
          {showTax && hoveredDay.deadlines.length > 0 && (
            <div className="flex flex-col gap-1.5">
              <span className="text-[9px] font-black uppercase tracking-wider text-blue-400">📋 Налоговые дедлайны</span>
              <div className="flex flex-col gap-1.5 max-h-40 overflow-y-auto pr-1">
                {hoveredDay.deadlines.map((dl) => (
                  <div key={dl.id} className="text-[11px] leading-snug p-2 rounded-xl bg-white/[0.03] border border-white/5 flex flex-col gap-0.5">
                    <div className="flex items-start gap-1 font-bold text-slate-200">

...
export default function TxModal({ tx, defaultType, companyId, onSave, onDelete, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-bx-surface border border-bx-border-2 rounded-2xl w-[440px] shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-bx-border">
          <h2 className="text-base font-semibold text-bx-text">{isEdit ? 'Редактировать операцию' : 'Новая операция'}</h2>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => setType('income')}
              className={`flex-1 py-2 text-sm rounded-lg transition-colors ${type === 'income' ? 'bg-emerald-600 text-white' : 'bg-bx-surface-2 text-bx-muted hover:text-bx-text'}`}>↑ Доход</button>
            <button onClick={() => setType('expense')}
              className={`flex-1 py-2 text-sm rounded-lg transition-colors ${type === 'expense' ? 'bg-red-600 text-white' : 'bg-bx-surface-2 text-bx-muted hover:text-bx-text'}`}>↓ Расход</button>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] text-bx-muted block mb-1">Сумма</label>
              <input autoFocus type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0" className={field}
                onKeyDown={e => { if (e.key === 'Enter') save(); }} />
            </div>
            <div>
              <label className="text-[10px] text-bx-muted block mb-1">Валюта</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            {currency !== 'UZS' && (
              <div>
                <label className="text-[10px] text-bx-muted block mb-1">Курс к UZS</label>
            ...
            <div className={currency === 'UZS' ? 'col-span-2' : ''}>
              <label className="text-[10px] text-bx-muted block mb-1">Дата</label>
              <input type="date" value={date} onChange={e => setDate(e.target.value)} className={field} />
            </div>
          </div>

          <div>
            <label className="text-[10px] text-bx-muted block mb-1">Категория</label>
          ...
          <div>
            <label className="text-[10px] text-bx-muted block mb-1">Контрагент</label>
            <input value={counterparty} onChange={e => setCounterparty(e.target.value)} placeholder="Клиент / поставщик" className={field} />
          </div>

          <div>
            <label className="text-[10px] text-bx-muted block mb-1">Комментарий</label>
          ...
          <div className="flex gap-2">
            <button onClick={() => setStatus('paid')}
              className={`flex-1 py-1.5 text-xs rounded-lg transition-colors ${status === 'paid' ? 'bg-blue-600/30 text-blue-300 border border-blue-500/40' : 'bg-bx-surface-2 text-bx-muted'}`}>✓ Оплачено</button>
            <button onClick={() => setStatus('unpaid')}
              className={`flex-1 py-1.5 text-xs rounded-lg transition-colors ${status === 'unpaid' ? 'bg-amber-600/30 text-amber-300 border border-amber-500/40' : 'bg-bx-surface-2 text-bx-muted'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-bx-border">
          <div>
            {isEdit && onDelete && (
              confirmDel
                ? <div className="flex items-center gap-2 text-xs"><span className="text-red-400">Удалить?</span><button onClick={onDelete} className="px-2 py-1 bg-red-500/20 text-red-400 rounded-lg">Да</button><button onClick={() => setConfirmDel(false)} className="text-bx-muted">нет</button></div>

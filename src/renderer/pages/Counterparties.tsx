...
export default function Counterparties() {
  ...
  async function deleteSelectedCompany() {
    if (!selectedCompany) return
    const confirmed = window.confirm(`Удалить компанию «${selectedCompany.name}»? Будут потеряны её настройки и связи внутри BX. Это действие нельзя отменить.`)
    if (!confirmed) return
    await removeCompany(selectedCompany.id)
    localStorage.removeItem(`bx_company_details_${selectedCompany.id}`)
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-4">
        ...
        {loading && !showingCompanies ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="h-48 animate-pulse rounded-[20px] border border-bx-border bg-bx-surface" />)}</div> : showingCompanies ? (
          visibleCompanies.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{visibleCompanies.map(company => {
            const companyDetails = loadCompanyDetails(company.id)
            const completion = companyDetailsCompletion(companyDetails)
            return <button type="button" key={company.id} onClick={() => { setActive(company); navigate(`/companies/${company.id}`) }} className="group flex min-h-52 flex-col rounded-[20px] border border-bx-border bg-bx-surface p-4 text-left shadow-sm outline-none transition-colors hover:border-blue-500/35 focus-visible:ring-2 focus-visible:ring-blue-500">
              <div className="flex items-start justify-between gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-blue-500/10 text-blue-600 dark:text-blue-300"><Icon name="building" className="h-5 w-5" /></span><span className={`rounded-full border px-2.5 py-1 text-[9px] font-black ${company.id === active?.id ? 'border-emerald-500/25 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'border-bx-border bg-bx-surface-2 text-bx-muted'}`}>{company.id === active?.id ? 'Активная' : 'Подключена'}</span></div>
              <h2 className="mt-4 text-sm font-black text-bx-text group-hover:text-blue-600 dark:group-hover:text-blue-300">{company.name}</h2><p className="mt-1 text-[11px] text-bx-muted">ИНН {company.inn || 'не указан'} · {company.regime || 'режим не указан'}</p>
              <div className="mt-auto pt-4"><div className="flex justify-between text-[10px] font-bold text-bx-muted"><span>Готовность реквизитов</span><span>{completion.percent}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-bx-surface-2"><div className="h-full rounded-full bg-blue-600" style={{ width: `${completion.percent}%` }} /></div><span className="mt-3 flex items-center gap-1 text-[10px] font-black text-blue-600 dark:text-blue-300">Открыть компанию <Icon name="arrowR" className="h-3.5 w-3.5" /></span></div>
            </button>
...
function CompanyDetail({ company, details, completion, onBack, onEdit, onChange, onDelete, onNavigate }: {
  ...
  return <div className="flex flex-1 overflow-y-auto bg-bx-bg text-bx-text custom-scrollbar"><div className="bx-page-container w-full space-y-5 py-5">
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
      <div className="space-y-4"><section className="rounded-[20px] border border-bx-border bg-bx-surface p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-base font-black">Реквизиты для документов</h2><p className="mt-1 text-[11px] text-bx-muted">Сохраняются автоматически и используются в шаблонах.</p></div><span className={`rounded-full border px-3 py-1 text-[10px] font-black ${completion.percent === 100 ? 'border-emerald-500/25 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'border-amber-500/25 bg-amber-500/10 text-amber-700 dark:text-amber-300'}`}>{completion.percent}% готово</span></div><div className="mt-5 grid gap-4 md:grid-cols-2">{fields.map(([key, label, placeholder]) => <label key={key} className={labelClass}>{label}<input value={details[key]} onChange={event => onChange(key, key === 'mfo' ? event.target.value.replace(/\D/g, '').slice(0, 5) : key === 'account' ? event.target.value.replace(/\D/g, '').slice(0, 20) : event.target.value)} placeholder={placeholder} className={`${inputClass} mt-1.5`} /></label>)}</div><label className={`${labelClass} mt-4`}>Служебные заметки<textarea value={details.notes} onChange={event => onChange('notes', event.target.value)} rows={4} placeholder="Льготы, особенности работы, порядок согласования…" className={`${inputClass} mt-1.5 resize-y py-3`} /></label></section><section className="rounded-[20px] border border-bx-border bg-bx-surface p-5 shadow-sm"><CompanyTeamPanel /></section></div>
      <aside className="space-y-4"><section className="rounded-[20px] border border-bx-border bg-bx-surface p-4 shadow-sm"><h2 className="text-sm font-black">Готовность компании</h2><div className="mt-3 h-2 overflow-hidden rounded-full bg-bx-surface-2"><div className="h-full rounded-full bg-blue-600" style={{ width: `${completion.percent}%` }} /></div><p className="mt-3 text-[11px] leading-relaxed text-bx-muted">{completion.missing.length ? `Осталось добавить: ${completion.missing.join(', ')}.` : 'Основные реквизиты заполнены и готовы к автоподстановке.'}</p></section><section className="rounded-[20px] border border-bx-border bg-bx-surface p-4 shadow-sm"><h2 className="text-sm font-black">Связанные разделы</h2><div className="mt-3 space-y-2">{[['templates', 'Создать документ', '/templates'], ['note', 'Архив документов', '/documents'], ['finance', 'Контроль оплат', '/finance'], ['planner', 'Планировщик', '/planner']].map(([icon, label, path]) => <button type="button" key={path} onClick={() => onNavigate(path)} className="flex min-h-11 w-full items-center gap-3 rounded-xl border border-bx-border bg-bx-bg px-3 text-left text-xs font-bold hover:border-blue-500/30 hover:text-blue-600 dark:hover:text-blue-300"><Icon name={icon} className="h-4 w-4" />{label}<Icon name="arrowR" className="ml-auto h-3.5 w-3.5" /></button>)}</div></section><button type="button" onClick={onDelete} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-red-500/20 bg-red-500/10 text-xs font-bold text-red-600 hover:bg-red-500/15 dark:text-red-300"><Icon name="trash" className="h-4 w-4" />Удалить компанию</button></aside>
    </div>
  </div></div>
}
...
function CounterpartyDetail({ counterparty, form, saving, onBack, onChange, onSave, onDelete, onNavigate }: {
  ...
  const fields: Array<[keyof NewCounterparty, string, string]> = [['name', 'Название организации', 'ООО «Контрагент»'], ['inn', 'ИНН', '9 цифр'], ['phone', 'Телефон', '+998…'], ['address', 'Юридический адрес', 'Город, район, улица'], ['mfo', 'МФО', '5 цифр'], ['bank_name', 'Банк', 'Заполнится по МФО'], ['bank_account', 'Расчётный счёт', '20 цифр']]
  return <div className="flex flex-1 overflow-y-auto bg-bx-bg text-bx-text custom-scrollbar"><div className="bx-reading-container w-full space-y-5 py-5"><button type="button" onClick={onBack} className="inline-flex min-h-9 items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-bx-muted hover:text-bx-text"><Icon name="arrowL" className="h-3.5 w-3.5" />Все организации</button><header className="rounded-[24px] border border-bx-border bg-bx-surface p-5 shadow-sm"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-blue-600 dark:text-blue-300">{counterparty ? 'Внутренняя карточка контрагента' : 'Новая организация'}</p><h1 className="mt-1 text-2xl font-black">{counterparty?.name || 'Заполните реквизиты'}</h1><p className="mt-1 text-xs text-bx-muted">Данные используются в шаблонах документов и контроле оплат.</p></div><span className={`rounded-full border px-3 py-1.5 text-[10px] font-black ${health.tone === 'good' ? 'border-emerald-500/25 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'border-amber-500/25 bg-amber-500/10 text-amber-700 dark:text-amber-300'}`}>{health.percent}% заполнено</span></div></header><div className="grid gap-4 lg:grid-cols-[1fr_300px]"><section className="rounded-[20px] border border-bx-border bg-bx-surface p-5 shadow-sm"><h2 className="text-base font-black">Основные и банковские реквизиты</h2><p className="mt-1 text-[11px] text-bx-muted">Изменения применятся после сохранения карточки.</p><div className="mt-5 grid gap-4 md:grid-cols-2">{fields.map(([key, label, placeholder]) => <label key={key} className={`${labelClass} ${key === 'name' || key === 'address' ? 'md:col-span-2' : ''}`}>{label}{key === 'name' || key === 'inn' ? ' *' : ''}<input value={String(form[key] ?? '')} onChange={event => onChange(key, event.target.value)} placeholder={placeholder} className={`${inputClass} mt-1.5`} /></label>)}</div><div className="mt-5 flex flex-wrap gap-2"><button type="button" disabled={saving} onClick={onSave} className={primaryActionClass}><Icon name="save" className="h-4 w-4" />{saving ? 'Сохраняю…' : 'Сохранить карточку'}</button>{onDelete && <button type="button" onClick={() => void onDelete()} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-red-500/20 bg-red-500/10 px-4 text-xs font-bold text-red-600 hover:bg-red-500/15 dark:text-red-300"><Icon name="trash" className="h-4 w-4" />Удалить</button>}</div></section><aside className="space-y-4"><section className="rounded-[20px] border border-bx-border bg-bx-surface p-4 shadow-sm"><h2 className="text-sm font-black">Проверка карточки</h2>{health.missing.length ? <p className="mt-2 text-[11px] leading-relaxed text-bx-muted">Рекомендуется добавить: {health.missing.join(', ')}.</p> : <p className="mt-2 text-[11px] text-emerald-700 dark:text-emerald-300">Карточка готова для документов и оплат.</p>}</section><section className="rounded-[20px] border border-bx-border bg-bx-surface p-4 shadow-sm"><h2 className="text-sm font-black">Что можно сделать</h2><div className="mt-3 space-y-2">{[['templates', 'Создать документ', '/templates'], ['finance', 'Добавить оплату', '/finance'], ['search', 'Проверить ИНН', '/tools']].map(([icon, label, path]) => <button type="button" key={label} onClick={() => onNavigate(path)} className="flex min-h-11 w-full items-center gap-2 rounded-xl border border-bx-border bg-bx-bg px-3 text-left text-xs font-bold hover:border-blue-500/30 hover:text-blue-600"><Icon name={icon} className="h-4 w-4" />{label}<Icon name="arrowR" className="ml-auto h-3.5 w-3.5" /></button>)}</div></section></aside></div></div></div>
}

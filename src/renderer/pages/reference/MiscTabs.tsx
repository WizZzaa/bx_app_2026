...
import { regions, vedItems, statItems, dutyItems, penaltyItems } from '../../data/reference/misc';
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-[#1e2535] bg-[#141820] overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {regions.map(r => (
              <tr key={r.name} className="border-b border-[#1e2535] last:border-0 hover:bg-[#1a2030]">
...
function SimpleList({ rows }: { rows: { left: string; right: string; note?: string }[] }) {
  return (
    <div className="rounded-xl border border-[#1e2535] bg-[#141820] overflow-hidden">
      {rows.map((r, i) => (
        <div key={i} className="flex items-center gap-3 px-4 py-2.5 text-sm border-b border-[#1e2535] last:border-0 hover:bg-[#1a2030]">

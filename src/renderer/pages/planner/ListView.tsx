    ...
    <div className="flex flex-col h-full gap-3">
      ...
      <div className="flex gap-2 flex-wrap items-center flex-shrink-0">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Поиск..."
          className="bg-[#0f1117] text-slate-200 px-3 py-1.5 rounded-lg border border-[#2a3447] focus:outline-none focus:border-blue-500/50 text-sm w-48" />

        <select value={typeF} onChange={e => setTypeF(e.target.value as EventType | '')}
          className="bg-[#0f1117] text-slate-400 px-2.5 py-1.5 rounded-lg border border-[#2a3447] text-xs focus:outline-none">
        ...
        <span className="text-xs text-slate-600 ml-auto">{filtered.length} записей</span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#1e2535]">
              ...
              <th className="py-2 px-2 text-left text-xs text-slate-500 font-medium w-8">Тип</th>
              <th className="py-2 px-2 text-left text-xs text-slate-500 font-medium cursor-pointer hover:text-slate-300 select-none" onClick={() => thClick('title')}>Название{arrow('title')}</th>
              <th className="py-2 px-2 text-left text-xs text-slate-500 font-medium cursor-pointer hover:text-slate-300 select-none" onClick={() => thClick('status')}>Статус{arrow('status')}</th>
              <th className="py-2 px-2 text-left text-xs text-slate-500 font-medium cursor-pointer hover:text-slate-300 select-none" onClick={() => thClick('priority')}>Приор.{arrow('priority')}</th>
              <th className="py-2 px-2 text-left text-xs text-slate-500 font-medium cursor-pointer hover:text-slate-300 select-none" onClick={() => thClick('date')}>Дата{arrow('date')}</th>
              <th className="py-2 px-2 text-left text-xs text-slate-500 font-medium">Теги</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(ev => (
              <tr key={ev.id}
                className={`border-b border-[#1e2535]/50 hover:bg-[#1e2535]/40 transition-colors cursor-pointer ${isOverdue(ev) ? 'bg-red-900/5' : ''}`}>
                ...
                <td className="py-2 px-2 max-w-xs" onClick={() => onEdit(ev)}>
                  <p className={`text-slate-200 truncate ${isOverdue(ev) ? 'text-red-300' : ''}`}>{ev.title}</p>
                  {ev.note && <p className="text-[10px] text-slate-600 truncate">{ev.note}</p>}
                </td>

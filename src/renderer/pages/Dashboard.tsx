...
export default function Dashboard() {
  ...
  const updateVisible = (next: ServiceVisibility) => { setVisible(next); localStorage.setItem('bx_dashboard_widgets', JSON.stringify(next)) }
  ...
  return (
    <main className="z-10 flex-1 overflow-y-auto bg-bx-bg px-5 py-5 text-bx-text sm:px-6">
      <div className="bx-page-container space-y-4">
        ...
        {configOpen && <section className="flex flex-wrap items-center gap-2 rounded-2xl border border-bx-border bg-bx-surface p-3 shadow-sm"><span className="mr-2 text-[10px] font-extrabold uppercase tracking-wide text-bx-muted">Сервисные блоки</span>{([['weather', 'Погода'], ['notifications', 'Оповещения'], ['horoscope', 'Бухо-гороскоп']] as const).map(([key, label]) => <label key={key} className="flex min-h-9 cursor-pointer items-center gap-2 rounded-xl bg-bx-bg px-3 text-xs font-bold text-bx-text"><input type="checkbox" checked={visible[key]} onChange={event => updateVisible({ ...visible, [key]: event.target.checked })} className="accent-blue-600" />{label}</label>)}</section>}
        ...
        <section className="grid gap-4 xl:grid-cols-12">
          ...
          <article className="flex min-h-[430px] flex-col rounded-[26px] border border-bx-border bg-bx-surface p-5 shadow-sm xl:col-span-3">
            ...
            <div className="mt-auto grid grid-cols-2 gap-2 pt-4"><QuickAction icon="news" label="Новости" onClick={() => navigate('/news')} /><QuickAction icon="ai" label="Спросить AI" onClick={() => navigate('/ai')} /><QuickAction icon="calc" label="Расчёты" onClick={() => navigate('/calc')} /><QuickAction icon="templates" label="Шаблоны" onClick={() => navigate('/templates')} /></div>
          </article>
        </section>

...
- [ ] `/dashboard`: новый task-first hero без пустых блоков.
- [ ] Фокус дня, рабочий ритм и ближайший срок.
- [ ] Новый бухгалтерский календарь с официальным статусом данных.
- [ ] Погода и курсы как компактные сигналы, а не отдельные тяжёлые панели.
- [ ] Бухо-гороскоп как спокойная необязательная пауза.
- [ ] Loading, empty, offline, stale, partial и error states.

## A2 — Планировщик и все формы задач

- [ ] `/planner`: календарь, список, фокус, доски, архив и системные задачи.
- [ ] Новая задача/событие: `EventModal`.
- [ ] Карточка задачи: `CardModal`.
- [ ] Доска: `BoardModal`.
- [ ] Задачи дня: `DailyTasksModal`.
- [ ] Timeline и summary события.
- [ ] Drag/drop, keyboard, mobile sheet и reduced-motion.

## A3 — Документы

- [ ] `/documents/templates`: каталог, категории, поиск и preview.
- [ ] Создание и редактирование шаблона.
- [ ] `/documents/my`: список, загрузка, фильтры, карточка и состояния обработки.
- [ ] PDF/DOCX/XLSX preview, export и demand-loading.
- [ ] Ошибки файлов, прогресс, отмена и безопасное восстановление.

## A4 — AI и переводчик

- [ ] `/ai`: новый разговорный workspace, источники, история, квота и recovery.
- [ ] Composer, attachment flow, consent и provider states.
- [ ] `/translator`: финальная унификация с Apple-style shell.
- [ ] Первый запуск переводчика без постоянной кнопки «Показывать обучение».
- [ ] Загрузка документа, языки, настройки, результат, повтор и экспорт.

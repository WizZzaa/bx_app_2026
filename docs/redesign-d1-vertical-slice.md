...
`ui-ux-pro-max` предложил Bento Grid, Financial Dashboard, 150–300 мс для микровзаимодействий, WCAG AA и mobile-first. Предложенные им внешние Fira/Plus Jakarta Sans и тёмно-зелёная палитра не принимаются: они конфликтуют со спокойным B2B-направлением мастер-спецификации и каноническим BX-контрактом. Из рекомендации принимаются модульная Bento-иерархия, нейтральные поверхности, один акцент, явные финансовые статусы, адаптивный reflow и строгие accessibility/performance gates. Канонический шрифт — self-hosted Geist с системным fallback; до поставки проверенных локальных font assets runtime продолжает использовать текущий системный стек, чтобы D1-A не создавал сетевую зависимость и layout shift.
...
|---|---|---|---|
| `--bx-canvas` | `#F7F6FA` | `#111018` | общий фон |
| `--bx-surface` | `#FFFFFF` | `#181621` | рабочая панель/карточка |
| `--bx-surface-subtle` | `#F1EFF6` | `#211E2B` | secondary control, selected row |
| `--bx-surface-strong` | `#E5DFEC` | `#2A2636` | raised/pressed surface |
| `--bx-text-primary` | `#27242F` | `#F7F4FB` | 14.15:1 / 17.36:1 к canvas |
| `--bx-text-secondary` | `#6E6877` | `#C2BACD` | 4.99:1 / 10.08:1 к canvas |
| `--bx-border` | `#DED9E8` | `#3D374A` | разделитель, не единственный сигнал |
| `--bx-brand` | `#7568D8` | `#B9A2E2` | канонический акцент из `DESIGN.md` |
| `--bx-brand-action` | `#6659C8` | `#B9A2E2` | заполненный CTA с AA-текстом |
| `--bx-brand-soft` | `#EEEAFB` | `#2A2636` | выбранная/мягкая акцентная поверхность |
| `--bx-on-brand` | `#FFFFFF` | `#1B1329` | текст на `brand-action` |
| `--bx-success` | `#2F7D58` | `#57CC8A` | всегда с текстом/иконкой |
| `--bx-warning` | `#A96914` | `#F0B45A` | icon/border; для мелкого текста используется `--bx-warning-text` |
| `--bx-warning-text` | `#8A5A00` | `#F0B45A` | доступный warning text |
| `--bx-danger` | `#B53A45` | `#FF8A80` | destructive/error |
| `--bx-info` | `#52677A` | `#75B8FF` | информационный статус |
| `--bx-focus` | `#7568D8` | `#D3C1F2` | focus ring 3 px + offset 2 px |
...
| Группа | Токены |
|---|---|
| Шрифт | `Geist, Inter, "Segoe UI", Roboto, Arial, sans-serif`; Geist поставляется только self-hosted, mono — текущий системный стек |
...
| Radius | `8 / 12 / 16 / 20 / 24 / pill`; control 12, nested surface 16, card 20, Bento hero 24 |
...
| Measure | рабочий max `1200px`, текстовый max `72ch` |
...
- 1440: 12 колонок, max-width 1200 px, gutter 32 px, gap 16 px.
...
Dependency preflight повторно выполнен 20 июля 2026 года: актуальные `motion` и `framer-motion` имеют версию `12.42.2`, peer range `react/react-dom ^18 || ^19` совместим с текущим React `19.2.7`. Пакет `motion` имеет unpacked size 682 296 байт/30 файлов, но транзитивно подключает `framer-motion` (4 787 682 байта/382 файла), поэтому размер npm-пакета не считается bundle-доказательством.

Изолированный minified esbuild spike с внешними React/ReactDOM показал 27 413 байт gzip для `LazyMotion + domAnimation + motion/react-m` и 3 667 байт gzip для `useAnimate` из `motion/react-mini`; baseline entry — 510 байт gzip. Первый вариант превышает D1-лимит 25 КБ и заблокирован, второй проходит размерный gate и остаётся кандидатом для React adapter. Это предварительный относительный замер, а не замена production Vite manifest: в D1-A зависимость намеренно не установлена, сначала поставляются framework-neutral presets и их тесты, затем adapter проходит повторный App/Web production build и обязан дать ≤25 КБ gzip на использующем route и 0 КБ на route без motion.
...
Adapter по умолчанию использует `useAnimate` из `motion/react-mini`; декларативный `LazyMotion + domAnimation + m` допускается только после отдельной оптимизации, так как текущий spike не прошёл лимит. На экране не больше двух заметных motion-акцентов. Бесконечная анимация допустима только для реального progress/loading indicator.
...
| 1440×900 | 12-column Bento, max-width 1200, стабильный Sidebar |
